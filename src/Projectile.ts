...
import { CollisionEventArgs } from "./engine/utils";
import { invisible_part } from "./main";

import { Shape } from "./engine/Shape";
import { Vector2D } from "./engine/vector2D";
import { Emitter, EventArgs } from "./engine/utils";
import { BodyComponent } from "./engine/ECS/BodyComponent";
import { Entity } from "./engine/ECS/ECS";
...
export class Projectile extends Actor {
  ...
  update(deltaTime: number): void {
    ...
    this._bodyComponent.position = this._bodyComponent.position.add(
      this.direction.multiply(this.speed * deltaTime),
    );
    if (
      this._bodyComponent.position.x > window.innerWidth ||
      this._bodyComponent.position.y > window.innerHeight ||
      this._bodyComponent.position.x < 0 ||
      this._bodyComponent.position.y < 0
    ) {
    ...
    this.log_pos += ` x: ${this._bodyComponent.position.x} ; y: ${this._bodyComponent.position.x} \n`;
  }
  ...
  draw() {
    ...
    ctx.beginPath();
    ctx.arc(
      this._bodyComponent.position.x,
      this._bodyComponent.position.y,
      5,
      0,
      2 * Math.PI,
    );
  ...
  notified<T extends Emitter, U extends EventArgs>(emitter: T, Args: U): void {
    console.log("j'ai été notifiée");
    const isCollisionEventArg = (arg: EventArgs): arg is CollisionEventArgs => {
      const colArg = arg as CollisionEventArgs;
      return !!colArg?.other;
    };
    if (emitter instanceof BodyComponent && isCollisionEventArg(Args)) {
      this.onCollision(Args.other.entity as Actor);
    }
  }
  ...
  onCollision(other: Actor): void {
    if (other.name === "enemy") {
      ...
      console.log(this.manager.collidables.length);
      console.log(
        `Collision avec un Enemy -> x:${other._bodyComponent.position.x}, y:${other._bodyComponent.position.y}`,
      );
      enemy.takeDamage(5);
